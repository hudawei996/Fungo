...
class CircleProgressView : ProgressBar {
    ...
    private var mReachBarSize = dp2px(context, 2) // 未完成进度条大小
    private var mNormalBarSize = dp2px(context, 2) // 未完成进度条大小
    private var mReachBarColor = Color.parseColor("#108ee9") // 已完成进度颜色
    private var mNormalBarColor = Color.parseColor("#FFD3D6DA") // 未完成进度颜色
    private var mTextSize = sp2px(context, 14) // 进度值字体大小
    ...
    private var mRadius = dp2px(context, 20) // 半径
    ...
    private var mInnerPadding = dp2px(context, 1) // 内部圆与外部圆间距
    ...
    private var mOuterSize = dp2px(context, 1) // 外层圆环宽度
    ...
    fun setReachBarSize(reachBarSize: Int) {
        mReachBarSize = dp2px(context, reachBarSize)
        invalidate()
    }
    ...
    fun setNormalBarSize(normalBarSize: Int) {
        mNormalBarSize = dp2px(context, normalBarSize)
        invalidate()
    }
    ...
    fun setInnerPadding(innerPadding: Int) {
        mInnerPadding = dp2px(context, innerPadding)
        val mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding
        rectInner = RectF((-mInnerRadius).toFloat(), (-mInnerRadius).toFloat(), mInnerRadius.toFloat(), mInnerRadius.toFloat())
        invalidate()
    }
    ...
    fun setOuterSize(outerSize: Int) {
        mOuterSize = dp2px(context, outerSize)
        invalidate()
    }
