...
class SocialActivity : BaseActivity() {
    ...
    private var shareMedia: IShareMedia = ShareTextMedia("默认文字")
            ...
            when (checkedId) {
                ...
                R.id.rbTypeImage -> {
                    loadBitmap(this, mImageUrl, object : OnImageListener {
                        override fun onSuccess(bitmap: Bitmap?) {
                            if (bitmap != null) {
                                shareMedia = ShareImageMedia(bitmap)
                            }
                        }
                ...
                R.id.rbTypeTextImage -> {
                    loadBitmap(this, mImageUrl, object : OnImageListener {
                        override fun onSuccess(bitmap: Bitmap?) {
                            if (bitmap!=null) {
                                 shareMedia = ShareTextImageMedia(bitmap,getString(R.string.share_text))
                            }
                        }
                ...
                R.id.rbTypeLink -> {
                    loadBitmap(this, mImageUrl, object : OnImageListener {
                        override fun onSuccess(bitmap: Bitmap?) {
                            if (bitmap!=null) {
                                shareMedia = ShareWebMedia(mShareUrl,getString(R.string.share_title),getString(R.string.share_text),bitmap)
                            }
                        }
