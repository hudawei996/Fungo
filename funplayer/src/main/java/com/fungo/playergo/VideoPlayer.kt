...
class VideoPlayer @JvmOverloads constructor(private val mContext: Context, attrs: AttributeSet? = null) : FrameLayout(mContext, attrs), IVideoPlayer, TextureView.SurfaceTextureListener {
    ...
    private fun initMediaPlayer() {
        if (mVideoExecutor == null) {
            mVideoExecutor = VideoPlayerManager.instance.getVideoPlayer(context)
    ...
    override fun enterFullScreen() {
        ...
        VideoPlayerUtils.scanForActivity(mContext)!!.requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE

        val contentView = VideoPlayerUtils.scanForActivity(mContext)!!
    ...
    override fun exitFullScreen(): Boolean {
        if (mCurrentMode == MODE_FULL_SCREEN) {
            ...
            VideoPlayerUtils.scanForActivity(mContext)!!.requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
    ...
    override fun enterTinyWindow() {
        ...
        val params = FrameLayout.LayoutParams(
                (VideoPlayerUtils.getScreenWidth(mContext) * 0.6f).toInt(),
                (VideoPlayerUtils.getScreenWidth(mContext).toFloat() * 0.6f * 9f / 16f).toInt())
    ...
    private val mOnPreparedListener = object : IVideoExecutor.OnPreparedListener {
        override fun onPrepared(executor: IVideoExecutor) {
            ...
            if (continueFromLastPosition) {
                val savedPlayPosition = VideoPlayerUtils.getSavedPlayPosition(mContext, mUrl!!)
