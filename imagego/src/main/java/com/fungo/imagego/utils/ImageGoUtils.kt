...
object ImageGoUtils {
    ...
    fun getImagePath(context: Context?): String {
        val path = getAppDataPath(context) + File.separator + ImageGoConfig.PATH_IMAGE + File.separator
    ...
    fun copyFile(srcFile: File?,
        ...
        return try {
            ...
            val data = ByteArray(ImageGoConfig.sBufferSize)
            var len = stream.read(data, 0, ImageGoConfig.sBufferSize)
            while (len != -1) {
                os.write(data, 0, len)
                len = stream.read(data, 0, ImageGoConfig.sBufferSize)
            }
            true
